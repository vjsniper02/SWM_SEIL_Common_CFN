...
def arrange():
    ...
    client = create_autospec(ProposalLineItemClient)
    container.proposal_lineitem_api_client.override(client)
...
def test_handler_calls_correct_service(arrange):
    ...
    # Assert
    client.create_proposal_lineitem.assert_called_once_with(event)
...
def test_handler_throws_correct_create_error(exception, expected_http_code, arrange):
    client = arrange
    client.create_proposal_lineitem.side_effect = exception("test")
...
def test_handler_throws_correct_update_error(exception, expected_http_code, arrange):
    client = arrange
    client.update_proposal_lineitem.side_effect = exception("test")
    ...
    event = {
        ...
        "transactionContext": {
            ...
            "correlationId": "postman-testing-1",
            "applicationLabel": "postman",
            "path": "/v1/gam/proposallineitem/1234",
            "method": "PUT",
        },
    }
    result = handler(event=event, context={})
    ...
    # Assert
    client.update_proposal_lineitem.assert_called_once_with(event)
    assert result["code"] == expected_http_code
