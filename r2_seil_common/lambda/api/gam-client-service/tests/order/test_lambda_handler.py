...
def arrange():
    ...
    client = create_autospec(OrderClient)
    container.order_api_client.override(client)
...
def test_handler_calls_correct_service(arrange):
    ...
    # Assert
    client.create_orders.assert_called_once_with(event)
...
def test_handler_throws_correct_error(exception, expected_http_code, arrange):
    ...
    client.create_orders.side_effect = exception("test")
