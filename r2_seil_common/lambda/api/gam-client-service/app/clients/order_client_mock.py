...
ORDER_SAMPLE_RESPONSE = {"id": 5062505, "name": "order-name-1", "status": "DRAFT"}
...
class OrderClientMock(OrderClient):
    ...
    def createOrders(self, create_order_request):
        return [ObjectFromDict(**ORDER_SAMPLE_RESPONSE)]

    def getOrdersByStatement(self, request):
        return {"results": [ORDER_SAMPLE_RESPONSE]}

    def performOrderAction(self, order_action, statement_query):
        return [ObjectFromDict(**ORDER_SAMPLE_RESPONSE)]
